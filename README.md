...
예를 들어 방송으로 철수와 영희를 부른다고 한다면

그 방송은 그곳에 있었던 모든 사람들에게 들리지만

당사자인 철수와 영희는 좀 더 집중해서 듣게 되는 그런 방식이다.
...
클라이언트가 요청을 보내고 답장이 올 때까지 멈추는 동기 방식과

요청을 보내고 답장이 오기 전 먼저 다음 코드를 수행하는 비동기 방식으로 나누어진다.
...
서버가 먼저 목표에 대해 수락했는지 거절했는지를 전송하고,

수락했다면 이후 중간중간 피드백을 전송하고
...
인터페이스 패키지가 아닌 다른 패키지(파이썬 패키지 등)의 경우는 

위 내용들이 적용되지 않으며,

만약 사용하려는 다른 패키지나 인터페이스 패키지가 있을 경우
